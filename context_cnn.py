...
class ResNet(nn.Module):
    ...
    def __init__(self, block=Bottleneck,  layers=[3, 4, 6, 3], nb_classes=40, channel=3):
        ...
        self.extractor = SingleRoIExtractor(dict(type='RoIAlign', out_size=3, sample_num=2),
                                            out_channels=1024,
                                            featmap_strides=[16,]
                                            )
    ...
    def forward(self, x,rois):
        ...
        #x4 = self.convx4(x4)
        mark2 = time.time()
        feature = x3.unsqueeze(0)
        roi_feats = self.extractor(feature,rois)
        #print(roi_feats.shape)
        roi_feats = self.convx(roi_feats)
        #print(roi_feats.shape)
        #print(x4.shape)
        #print(roi_feats)
        roi_feats = roi_feats.contiguous().view(-1,100,2048,1,1)
        newx4 = self.maxpool(x4)
        global_feat = newx4.view(-1,1,2048,1,1)
        roi_feats = torch.cat((global_feat,roi_feats),1)

        mark3 = time.time()
        #print("m3-2  ")
        #print(mark3-mark2)
        roi_feats_atn,relation = blockAttention(roi_feats,self.attention_op)
        mark4 = time.time()
        #print("4-3")
        #print(mark4-mark3)
        #idx = torch.argmax(relation[:,0,:],dim=1)
        #choose = idx#idx[:,0]
        b,n,c,h,w = roi_feats_atn.shape
        #feat = torch.zeros(b,2,c,h,w).cuda()
        #for jj in range(roi_feats_atn.shape[0]):
         #   feat[jj][0] = roi_feats_atn[jj][0]
         #   feat[jj][1] = roi_feats_atn[jj][choose[jj]]
        #feat = feat.view(-1,c,h,w)
        mark5 = time.time()
        global_feat_new = roi_feats_atn[:,0,:,:,:].unsqueeze(1)
        local_feat_new = roi_feats_atn[:,1:,:,:,:].view(b,n-1,c*h*w).permute(0,2,1)
        local_feat_new = self.channelpool(local_feat_new).permute(0,2,1).view(b,1,c,h,w)
        assert global_feat_new.shape == local_feat_new.shape
        feat = torch.cat((global_feat_new,local_feat_new),1)
        ...
        #feat = self.avgpool(feat).view(b,2,c)
