...
class ResNet(nn.Module):
    ...
    def __init__(self, block=Bottleneck,  layers=[3, 4, 6, 3], nb_classes=40, channel=3):
        ...
        self.extractor = SingleRoIExtractor(dict(type='RoIAlign', out_size=7, sample_num=1),
                                            out_channels=512,
                                            featmap_strides=[4,]
                                            )
    ...
    def forward(self, x,rois):
        ...
        #feature.append(x3)
        #feature.append(x4)
        feature = x2.unsqueeze(0)
        roi_feats = self.extractor(feature,rois)
        #print(roi_feats.shape)
        #x3 = self.layer3(roi_feats)
        #roi_feats = self.layer4(x3)
        b,c,h,w = roi_feats.shape
        roi_feats = roi_feats.view(-1,16,512,7,7)
        #512*7*7-->2048
        ...
        roi_feats_atn = roi_feats_atn.view(b*t,-1)
        #print(roi_feats_atn.shape)
        #(B,2048*2)
        #feat = torch.cat((global_feat,local_feat_new),1)
        feat =  roi_feats_atn
        out = F.softmax(self.fc_s(feat),1)
        return  out
