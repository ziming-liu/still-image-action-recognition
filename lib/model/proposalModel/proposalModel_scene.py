...
class proposalModel_scene(nn.Module):
    def __init__(self):
        super(proposalModel_scene, self).__init__()
        self.HIGHSCENE = [3,4,6,7,9,10, 13,12,11,16,24,25,26,32,39]
        self.backbone = ResNet(50,4,out_indices=(1,3,))# int obj不可迭代，加个 ,
    ...
    def forward(self, img,gtbboxes,proposals,label):
        #TODO:move the limit of one sample , support batch size train
        backbonefeat,backbonefeat_ = self.backbone(img)#  14 *14
        #print('stage 1 {}'.format(backbonefeat.shape))
        #print('stage 3 {}'.format(backbonefeat_.shape))
        #print(backbonefeat)
        ...
        #print(bbox_feat.shape)
        assert bbox_feat.shape[1]==1024
        bbox_feat = bbox_feat.view(bbox_feat.size(0),-1)
        #print(bbox_feat.shape)
        if label[0].data.cpu().numpy() in self.HIGHSCENE:
           output= self.fc_high(torch.cat((globalfeat,bbox_feat),1))
        else:
            output = self.fc_low(torch.cat((bbox_feat,bbox_feat),1))
        return output
