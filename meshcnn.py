...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=40):
        ...
        self.extractor = SingleRoIExtractor(dict(type='RoIAlign', out_size=2, sample_num=1),
                                            out_channels=2048,
                                            featmap_strides=[32, ]
                                            )
    ...
    def forward(self, x,fullimg):
        ...
        x = self.avgpool(x)
        b2,c2,h2,w2 = x.shape
        x = x.contiguous().view(b,t,c2,h2,w2).view(b,t,-1)
        x,context = self.atn_s4(x,x,x)
        #print("context {}".format(context.shape))
        x = x.contiguous().view(b,t,c2,h2,w2).contiguous().view(b2,c2,h2,w2)
        ...
        outlocal = F.softmax(x,1)
        x = self.conv1(fullimg)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        x = x.view(x.size(0),-1)
        x = self.fc_action(x)
        outglobal = F.softmax(x,1)
        out = outlocal + outglobal
        return out,context
